# 📇 知识库索引

> 入口总览：**[[00-DASHBOARD|🗺️ Dashboard（推荐首页）]]** · [[00-TODO|✅ 待办]] · 本页为完整索引。
> 本库按领域组织；每个领域有 `_MOC.md` 作为入口。Dataview 插件已安装，MOC/TODO/Dashboard 页自动更新。

## 🗂 领域

- [[AI/_MOC|AI]]
- [[ARM/_MOC|ARM]]
- [[Beyond-Tech/_MOC|Beyond-Tech]]
- [[Languages/_MOC|Languages]]
- [[OS/_MOC|OS]]
- [[asr_project/_MOC|asr_project]]
- [[autosar/_MOC|autosar]]
- [[coding_knowledge/_MOC|coding_knowledge]]
- [[desktop_apps/_MOC|desktop_apps]]
- [[development-theory/_MOC|development-theory]]
- [[Communication_BUS/_MOC|ethernet]]
- [[foreign/_MOC|foreign]]
- [[soft_models/_MOC|soft_models]]
- [[software_project/_MOC|software_project]]
- [[test_etc/_MOC|test_etc]]
- [[tool_chains/_MOC|tool_chains]]

## 📄 根目录笔记

- [[About Pre-compile & Post-build|About Pre-compile & Post-build.md]]
- [[0-layers|0-layers.md]]
- [[About-license|About-license.md]]
- [[design_lesson|design_lesson.md]]

## 🗃 归档与附件

- [[99-archive/_MOC|99-archive（归档，不删除）]]
- `90-attachments/`（附件：图片/PDF/视频）

## ✅ 待办总览

- [[00-TODO|TODO 总览]]

## ✍️ 新笔记模板

- `_templates/note-template.md`