# 路由原理：从 LAN A 经 WAN 到 LAN B

> 面向对象：熟悉嵌入式、TCP/IP 基础、局域网广播域和 ARP，但对跨网段路由过程不熟悉的工程师。  
> 核心问题：**Host A 发出的一个 IP 报文，如何经过路由器和 WAN，到达另一个局域网中的 Host B？**
...
```text
Ethernet:
    dst_mac = R1_LAN_MAC      ← 这一跳交给路由器 R1
...
> 目标主机就在 eth1 所连接的 LAN B 上，不需要再找第三台路由器。
...
更准确地说，不是“同一个 Ethernet Frame 穿过了路由器”，而是：
...
这一条规则同时解释了 LAN 通信和路由通信。

---

# 21. Longest Prefix Match：路由表怎么选
...
如果 TTL 变成 0，路由器丢弃报文，并通常返回：

```text
ICMP Time Exceeded
```

作用是防止路由环路导致 IP Packet 永远在网络中转圈。
...
> **路由属于三层逻辑；Ethernet、PPP 等只是承载这一跳 IP Packet 的链路技术。**
...
2. 路由表根据目标 IP 决定下一跳和出接口。

3. ARP/Neighbor 根据下一跳 IP 得到当前链路的二层地址。

4. 每经过一个路由器，旧二层帧结束，重新构造新的二层帧。

5. 在纯路由场景中，源/目标 IP 端到端保持不变，而 MAC 逐跳改变、TTL 逐跳递减。
```

如果这五句话能从头解释一遍，LAN A → WAN → LAN B 的基本路由原理就已经建立完整。
