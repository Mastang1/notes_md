...
>_**总结：
**`PN`**: 你的包名（Package Name）
yocto的实现流程[[#一、 认知基线：纯 Makefile vs 纯 Bitbake]], 核心实现 _**它先用内部脚本，自己从零编译出一套编译器和工具链（放入 `sysroots`），初始化基本的环境变量；

${D} 组件的临时构建路径的image路径，是个绝对路径；实际用的时候和其他路径拼接。
...
- **机制：** Yocto 认为**人类是不可靠的**，宿主机的环境是肮脏的。所以当你敲下 `bitbake virtual/kernel` 时：
    ...
    1. _**它先用内部脚本，自己从零编译出一套编译器和工具链（放入 `sysroots`）。
