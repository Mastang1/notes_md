_**Summary: 结构上：实现三点核心功能，一个是实现链接；一个是通过把dev放在首地址实现扩展私有成员（不同dev的千人千面）；最后是把运行时的 operations 都注册到了指定的结构体中，用统一的指针指向，mr_dev_ops & mr_rv**_
...
4 最终的效果就是：不同的设备OPS（一级多态实现，通过name扩展；主要实现了逻辑层；然后底层具体实现要求可适配，）实现了逻辑服务；然后可以调用drv->ops实现底层硬件的控制
