## 1. sysroot 机制
_**1. 一些初步的信息：
 - 1 存在一个集中存储依赖文件的路径 _**build/tmp/sysroot_components**_,当 lib-module 执行完 do_install 后，Yocto 的 do_populate_sysroot 任务会被触发。这个任务会把 ${D} 中属于开发类的目录（如 /usr/include, /usr/lib）复制或硬链接到这个中央仓库的对应子目录下。【本质就是一个仓库】；
