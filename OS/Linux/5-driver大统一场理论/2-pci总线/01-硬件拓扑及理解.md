...
- **RC (Root Complex)**：是连接 CPU 与 PCIe 世界的“逻辑枢纽”。_**它负责把 CPU 出来的 AXI 内存读写指令，翻译打包成 PCIe 的标准包裹（TLP）。
...
**你的结论 100% 正确：_**RC 和 RP 是逻辑管理与物理实例的关系，一个 RC 通常管理多个独立的 RP，且数量在芯片流片前就已经彻底固化。**
