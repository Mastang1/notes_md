...
- `SRC-IMX6ULL-DRV`：查目录中的 U-Boot/Kernel 移植与编译章节；今天只用于核对厂商 BSP 组织，不照抄固定路径。
- `SRC-ALIENTEK-UBOOT-REPO`：观察公开归档中的 `configs/`、`mx6ull_alientek_*.sh`、`.imx` 产物命名。
- `SRC-ALIENTEK-KERNEL-REPO`：观察 `arch/arm/configs/`、`arch/arm/boot/dts/`。
...
阅读目标不是懂 Makefile 全部语法，而是找到“配置入口”和“产物位置”。
