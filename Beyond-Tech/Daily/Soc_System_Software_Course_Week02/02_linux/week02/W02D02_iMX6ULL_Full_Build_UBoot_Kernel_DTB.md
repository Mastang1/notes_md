...
- `SRC-IMX6ULL-DRV`：系统移植/编译相关章节；用来核对你板型的厂商命令。
- `SRC-ALIENTEK-UBOOT-REPO`：公开归档证明正点原子 U-Boot 工程包含 `configs/` 与 `mx6ull_alientek_*.sh`。
- `SRC-ALIENTEK-KERNEL-REPO`：Kernel source layout。
